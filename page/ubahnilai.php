...
<form id="form" action="./proses/prosesubah.php" method="POST">
    ...
    <div class="panel-middle">
        ...
        <?php
        $query="SELECT namaKriteria,id_nilaisupplier,id_kriteria,id_nilaikriteria FROM nilai_supplier INNER JOIN kriteria USING(id_kriteria) WHERE id_supplier='$a'";
        $execute=$konek->query($query);
        if ($execute->num_rows > 0){
            while($data=$execute->fetch_array(MYSQLI_ASSOC)){
                ...
                $query2="SELECT id_nilaikriteria,id_kriteria,keterangan FROM nilai_kriteria WHERE id_kriteria='$data[id_kriteria]'";
                $execute2=$konek->query($query2);
                    if ($execute2->num_rows > 0){
                        while ($data2=$execute2->fetch_array(MYSQLI_ASSOC)){
                            ...
                            if ($data['id_nilaikriteria'] == $data2['id_nilaikriteria']) {
                                // echo "$data2[id_nilaikriteria]";
